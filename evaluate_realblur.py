...
dataset = 'RealBlur_J'
file_path = os.path.join('test_dir', f'pred_{dataset}')
# file_path = os.path.join('test_dir', f'pred_MPRNet_RS2J')
# file_path = '/mnt/q/networks/MIMO-UNet-main/results/MIMO-UNet/result_image'
# file_path = '/mnt/c/Users/95192/Desktop/Restormer/Motion_Deblurring/results/GoPro'
gt_path = os.path.join(f'/mnt/d/realblur/{dataset}/test', 'target')
...
assert  len(path_list) == len(gt_list), "predicted and target dataset length are not equal"
print(f'find {len(path_list)} images!')
psnr, ssim = [], []
...
f = open('simple.log', 'a')
f.write('real -> For {:s} dataset PSNR: {:f} SSIM: {:f}\n'.format(dataset, avg_psnr, avg_ssim))
f.write('#'*20)
f.write('\n')
f.close()
