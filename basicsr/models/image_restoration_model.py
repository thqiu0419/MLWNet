...
class ImageRestorationModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get("edge_opt"):
            ...
            self.cri_edge = cri_edge_cls(**train_opt['edge_opt']).to(
                self.device)
        else:
            self.cri_edge = False
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        if self.cri_pix:
            l_pix = 0.
            # for pred in preds:
            l_pix += self.cri_pix(preds, self.gt)[0]
        ...
        if self.cri_edge:
            l_edge = 0.
            l_edge += self.cri_edge(preds[0], self.gt)
            l_total += l_edge
    ...
    def auto_pad(self):
        pad_ratio = self.opt['val'].get('factor', 8)
        h, w = self.gt.shape[-2:]
        m_h, m_w = ((h + pad_ratio) // pad_ratio) * pad_ratio, ((w + pad_ratio) // pad_ratio) * pad_ratio
        pad_h = m_h - h if h % pad_ratio != 0 else 0
        pad_w = m_w - w if w % pad_ratio != 0 else 0
        self.lq = F.pad(self.lq, (0, pad_w, 0, pad_h), 'reflect')

    def get_current_visuals(self):
        out_dict = OrderedDict()
        ori_h, ori_w = self.gt.shape[-2:]
        out_dict['lq'] = self.lq[..., :ori_h, :ori_w].detach().cpu()
        out_dict['result'] = self.output[..., :ori_h, :ori_w].detach().cpu()
        if hasattr(self, 'gt'):
