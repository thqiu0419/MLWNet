...
from basicsr.models.archs.wavelet_block import LWN, FFTBlock, BasicConv
...
class Fusion(nn.Module):
    def __init__(self,
        ...
        self.d3 = nn.Sequential(*[NAFBlock(dim * 2 ** 2) for _ in range(num_blocks[2])])
        ...
        #                              # nn.GELU()
        #                              )
        self.d2 = nn.Sequential(*[NAFBlock(dim * 2) for _ in range(num_blocks[1])])
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.d4 = nn.Sequential(*[NAFBlock(dim * 2 ** 3) for _ in range(num_blocks[3])])
        self.d3 = nn.Sequential(*[NAFBlock(dim * 2 ** 2) for _ in range(num_blocks[2])])
        self.d2 = nn.Sequential(*[NAFBlock(dim * 2) for _ in range(num_blocks[1])])
        self.d1 = nn.Sequential(*[NAFBlock(dim) for _ in range(num_blocks[0])])
...
class MIMO_MLWNet(nn.Module):
    def __init__(self,
        ...
        super(MIMO_MLWNet, self).__init__()
